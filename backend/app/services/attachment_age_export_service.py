...
def _group_stats(rows: pd.DataFrame) -> tuple[int, float, float, float]:
    ...
    dataframe - so every summary number below is computed from the same
    rows the `data` sheet contains, not from a separately-called stat
    function."""
    case_count = len(rows)
    if case_count == 0:
...
def _write_stat_row(
    ws: Worksheet, row: int, label: str, delivery: str, stats: tuple[int, float, float, float]
) -> None:
    case_count, avg_minutes, min_minutes, max_minutes = stats
    ws.cell(row=row, column=1, value=label)
    ws.cell(row=row, column=2, value=delivery)
    ws.cell(row=row, column=3, value=case_count)
    for col, minutes in ((4, min_minutes), (5, avg_minutes), (6, max_minutes)):
...
def _build_overview_sheet(
    ...
    header_row = 4
    headers = ["Group", "Delivery", "Case Count", "Min", "Avg", "Max"]
    ...
    column_widths = [30, 16, 12, 12, 12, 12]
...
def _build_data_sheet(wb: Workbook, rows: pd.DataFrame) -> None:
    ...
    headers = ["IDs", "Case (Inborn/Outborn)", "attachment age (hh:mm:ss)", "Hours", "Minutes"]
    for col, header in enumerate(headers, start=1):
    ...
    for i, record in enumerate(ordered.to_dict(orient="records")):
        ...
        ws.cell(row=row, column=1, value=record["scr_babyid"])
        ...
        if minutes is None or pd.isna(minutes):
            ...
            if minutes_val >= 0:
                ...
                # Excel's built-in time formats cannot render a negative
                # duration - kept visible via the signed Hours/Minutes
                # columns instead of hiding the row or fabricating a value.
                ws.cell(row=row, column=3, value="N/A (negative)")
